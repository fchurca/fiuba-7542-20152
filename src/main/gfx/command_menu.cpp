...
void CommandMenu::draw() {
	...
	if (owner.font) {
		...
		texto = owner.completeLine(owner.player.name, size.x);
		for (auto r : owner.player.getResources()) {
			texto = texto + owner.completeLine(r.first + "=" + std::to_string(r.second), size.x);
		}
		int access1, w1, h1;
		Uint32 format1;
		SDL_Surface * c1 = TTF_RenderText_Blended_Wrapped(owner.font, texto.c_str(), colorBlanco, (Uint32)(size.x));
